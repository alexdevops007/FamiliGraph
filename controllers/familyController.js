...
const familyController = {
  ...
  createFamilyMember: async (req, res) => {
    try {
      ...
      console.error(error);
      res.status(500).json({
        message: "Erreur lors de la création du membre de la famille",
      });
    }
  },
  ...
  updateFamilyMember: async (req, res) => {
    try {
      const updatedFamilyMember = await Family.findByIdAndUpdate(
        req.params.id,
        req.body,
        { new: true }
      );
      ...
      console.error(error);
      res.status(500).json({
        message: "Erreur lors de la mise à jour du membre de la famille",
      });
    }
  },
  ...
  deleteFamilyMember: async (req, res) => {
    try {
      ...
      console.error(error);
      res.status(500).json({
        message: "Erreur lors de la suppression du membre de la famille",
      });
    }
  },
};
